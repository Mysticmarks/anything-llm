...
Two reusable harnesses now ship with the repository so CI and engineers can
hit the hot paths with identical parameters:

* **k6** scripts live under `server/benchmarks/k6.*.js` and expose the same
  environment variables as the Node harness plus latency/error budget
  thresholds (`*_P95_BUDGET_MS`, `*_P99_BUDGET_MS`). CI jobs can fail early
  when p95/p99 latency exceeds the budget while recording VU-concurrency and
  tags for queues.
* **Artillery** configs under `server/benchmarks/artillery.*.js` express
  arrival-rate driven phases. They accept the same workspace IDs/slugs and
  accept overrides through environment variables so pipelines can sweep
  concurrencies without mutating the files. Reports are JSON and suitable for
  CI artifacts and downstream charting.

Both harnesses emit queue tags (`chat`, `ingestion`, `agent_flow`) so metrics
dashboards can differentiate throughput by workload.

## Prometheus Scraping & Budgets

`/api/metrics/prometheus` now exports additional queue timing and circuit
budget metrics:

| Metric | Meaning |
| ------ | ------- |
| `anything_queue_duration_avg_ms` | Average runtime of completed queue tasks. |
| `anything_queue_duration_max_ms` | Maximum observed runtime per queue. |
| `anything_queue_saturation_total` | Counter of samples that exceeded the saturation threshold (default 85%). |
| `anything_circuit_error_budget_remaining` | Remaining attempts before the circuit trips open. |
| `anything_circuit_cooldown_ms` | Cooldown/backoff enforced before retrying the circuit. |

The CI Jest suite (`__tests__/metrics.test.js`) now scrapes these metrics to
assert that latency and saturation gauges render, while chaos tests
(`__tests__/chaos-resilience.test.js`) ensure crash/restart paths are exercised
for worker, Redis, and vector DB failures. These checks keep the Prometheus
surface area from regressing and force backoff logic to stay in place.

## Baseline Budgets & Artifacts

CI executions of the k6/Artillery harnesses should publish artifacts under
`artifacts/concurrency/` containing:

* TPS and p95/p99 latency charts for chat, ingestion, and agent flow runs using
  the configured concurrencies.
* Error budget deltas calculated from k6 thresholds and the Prometheus circuit
  counters.
* Queue saturation streaks to highlight headroom on the worker pools.

When running locally, the same files can be generated by passing `--out json`
to k6 or `--output` to Artillery and copying the resulting JSON into
`docs/server-concurrency.md` or Grafana dashboards. These baselines track the
impact of concurrency tweaks over time and guard against unnoticed regressions.
